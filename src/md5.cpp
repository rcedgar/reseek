...
https://thealgorithms.github.io/C-Plus-Plus/index.html
...
void MD5(const uint8_t *bytevec, uint64_t n, string &sigstr)
	{
	void *ptrSig = hashing::md5::hash_bs(bytevec, n);
	sigstr = hashing::md5::sig2hex(ptrSig);
	}

#if 0
static void Test(const char *s, const string &sigstr)
	{
	void* sig = hashing::md5::hash(s);
	string hex = hashing::md5::sig2hex(sig);
	ProgressLog("%s  string='%s'\n", s, hex.c_str());
	asserta(hex == sigstr);
	}
...
void cmd_md5()
	{
	uint64 n;
	const byte *Data = ReadAllStdioFile64(g_Arg1, n);
	...
	uint32_t Sig[4];
	MD5(Data, n, Sig);
	ProgressLog("MD5 %08lx%08lx%08lx%08lx\n",
