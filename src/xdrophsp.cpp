...
float DSSAligner::StaticSubstScore_Trace(void *UserData_this, uint PosA, uint PosB)
	{
	DSSAligner *pThis = (DSSAligner *) UserData_this;
	float Score = pThis->SubstScore(PosA, PosB);
	Log("Subst[%u,%u]=%.6g\n", PosA, PosB, Score);
...
float DSSAligner::XDropHSP_Trace(uint Loi_in, uint Loj_in, uint Len,
...
// Find highest-scoring 8mer
	...
	for (uint Col = 0; Col < Len; ++Col)
		v[Col] = StaticSubstScore_Trace((void *) this, Loi_in + Col, Loj_in + Col);
	...
	float TotalScore = m_XDropScoreFwd + m_XDropScoreBwd;
	if (TotalScore < 10)
		{
		m_XDropPath.clear();
		return 0;
