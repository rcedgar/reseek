...
SqueezeDelta *Squeezer::EncodePos_algo_tor2(
	...
	int8_t itheta_base = theta2i(theta_rad);
	int8_t iphi_base = phi2i(phi_rad);

	for (int ditheta = -2; ditheta <= +2; ++ditheta)
		{
		int8_t itheta = itheta_base + ditheta;
		for (int diphi = -2; diphi <= +2; ++diphi)
			{
			int8_t iphi = iphi_base + diphi;
