...
void cmd_checkfa2()
	...
	for (;;)
		...
		if (!Row1 && Ok)
			{
			asserta(SIZE(FirstRow) == SIZE(Row));
			fprintf(fOut, ">%s\n", FirstLabel.c_str());
			fprintf(fOut, "%s\n", FirstRow.c_str());
			fprintf(fOut, ">%s\n", Label.c_str());
			fprintf(fOut, "%s\n", Row.c_str());
	...
	ProgressLog("%u rows, %u not found, %u diffs\n",
		Rows, NotFound, Diff);
	CloseStdioFile(fOut);
	}
