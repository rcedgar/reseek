...
void ViterbiSetGaps(float Open, float Ext, float TermOpen, float TermExt)
	{
	asserta(Open >= 0 && Ext >= 0);
	asserta(TermOpen >= 0 && TermExt >= 0);
	s_Open = -Open;
	s_Ext = -Ext;
	s_TermOpen = -TermOpen;
	s_TermExt = -TermExt;
...
// Blosum62 on ASCII strings (not 0..19)
...
// Main loop
	...
	for (uint i = 0; i < LA; ++i)
		{
		const byte a = A[i];
		const float *MxRow = SubstMx[a].data();
		float Open = s_TermOpen;
		float Ext = s_TermExt;
		float I0 = MINUS_INFINITY;
...
float ViterbiFastMem_SubstMxPtr(
		XDPMem &Mem,
		const byte *A, uint LA,
		const byte *B, uint LB,
		const float *SubstMxVec,
		uint AlphaSize,
		string &Path)
	{
	if (LA*LB > 100*1000*1000)
...
// Main loop
	...
	for (uint i = 0; i < LA; ++i)
		{
		const byte a = A[i];
		const float *MxRow = SubstMxVec + AlphaSize*a;
		float Open = s_TermOpen;
		float Ext = s_TermExt;
		float I0 = MINUS_INFINITY;
