...
public:
	...
	void Run();
	void LogState() const;
	const char *xv2str(const vector<string> &xv, string &xstr) const;
	void GetBestVarStr(string &s) const;
	void GetBestVars(vector<double> &xs) const;
	void RunInitialValues();
	void RunLatin(vector<vector<string> > &xvs, vector<double> &ys);
	...
	const char *VarFloatToStr(uint VarIdx, double Value, string &s) const;
	...
	const char *VarSpecGetStr(uint VarIdx, const string &Name, 
		string &Str, const string &Default) const;

	const char *GetGlobalStr(const string &Name, string &s, const string &Default) const;
	...
	void DeltaVar(uint VarIdx, bool Plus, const string &OldValue,
		string &NewValue);
...
public:
	...
	static const char *GetRoundedStr(double x, uint SigFig, string &Str);
	...
	static const char *SpecGetStr(const string &Spec, const string &Name,
