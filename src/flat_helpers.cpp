...
void flat_union_alpha_names(
	...
	{
	set<string> name_set;
	for (size_t i = 0; i < varstrs.size(); ++i)
		{
		vector<string> alpha_names;
		flat_collect_alpha_names_from_varstr(varstrs[i], alpha_names);
		for (size_t j = 0; j < alpha_names.size(); ++j)
			name_set.insert(alpha_names[j]);
		}
	union_names.assign(name_set.begin(), name_set.end());
	sort(union_names.begin(), union_names.end());
	}
...
string flat_expand_varstr(
	...
	for (size_t i = 0; i < alpha_names.size(); ++i)
		NameToWeight[alpha_names[i]] = weights[i];

	string expanded;
	for (size_t i = 0; i < scalar_names.size(); ++i)
		Psa(expanded, "%s=%.10g;", scalar_names[i].c_str(), scalar_values[i]);
	for (size_t i = 0; i < union_names.size(); ++i)
		{
		const string &name = union_names[i];
		float w = 0;
		unordered_map<string, float>::const_iterator iter =
			NameToWeight.find(name);
		if (iter != NameToWeight.end())
			w = iter->second;
		Psa(expanded, "%s=%.10g;", name.c_str(), w);
		}
	return expanded;
	}
...
void flat_init_reseeker_params(
	...
	varstrs.push_back("=fold");
	varstrs.push_back("=sf");
	varstrs.push_back("=fam");
	vector<string> union_names;
	flat_union_alpha_names(varstrs, union_names);

	params_fold.init_from_varstr(flat_expand_varstr("=fold", union_names));
	params_sf.init_from_varstr(flat_expand_varstr("=sf", union_names));
	params_fam.init_from_varstr(flat_expand_varstr("=fam", union_names));

	flat_params::assert_compatible(params_fold, params_sf);
	flat_params::assert_compatible(params_fold, params_fam);
	}
...
void flat_build_query_mega_pssms(
	...
	for (uint qidx = 0; qidx < nquery; ++qidx)
		{
		const uint LQ = query_lengths[qidx];
		const uint8_t *prof = query_mega_profs[qidx];
		const uint nr_pssm_floats = LQ*params.m_sum_alpha_sizes;

		float *pssm = myalloc(float, nr_pssm_floats);
		fill_flat_pssm(
			prof, LQ, nfeat, alpha_sizes,
			params.m_feature_block_offsets,
			params.m_weighted_logoddsvec,
			pssm);
		query_mega_pssms[qidx] = pssm;

		float *pssm_rev = myalloc(float, nr_pssm_floats);
		fill_flat_pssm_reversed(
			prof, LQ, nfeat, alpha_sizes,
			params.m_feature_block_offsets,
			params.m_weighted_logoddsvec,
			pssm_rev);
		query_mega_pssm_revs[qidx] = pssm_rev;
		}
	}
