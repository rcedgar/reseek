...
void cmd_idx_search_kappa()
	...
	uint16_t *TBestScore = myalloc(uint16_t, Index.m_nseq);
	uint32_t *THitList = myalloc(uint32_t, Index.m_nseq);
	zero_array(TBestScore, Index.m_nseq);
	...
	for (uint qidx = 0; qidx < nquery; ++qidx)
		...
		auto NoteBest = [&](uint tidx, uint16_t diag)
			...
			++n_prehsp;
			if (f_prehsp != 0)
				kappa_filter::write_prehsp_hit(f_prehsp, QLabel,
					t_labels[tidx].c_str(), qidx, tidx, diag);

			const uint TL = t_lengths[tidx];
			if (TL < flat_params::m_kappa_min_chainlength)
				return;

			int DiagScore = ExtendDiagToHSP_DB(QSeq, QL, t_kappa[tidx], TL,
		...
		for (uint i = 0; i < n_thit; ++i)
			...
			TBestScore[tidx] = 0;
			kappa_filter::m_RSB.AddScore(qidx, tidx, sc);
			++n_hsp_accept;
			}
		}
	...
	ProgressLog("Kappa DB-index filter %u secs  prehsp=%llu  rsb_pairs=%u  hsp_targets=%llu\n",
		uint(t1 - t0),
		(unsigned long long) n_prehsp,
		total,
		(unsigned long long) n_hsp_accept);
